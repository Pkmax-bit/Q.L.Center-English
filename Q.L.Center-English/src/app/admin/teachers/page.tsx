...
export default function AdminTeachersPage() {
  ...
  const fetchTeachers = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: "Lỗi tải dữ liệu giáo viên", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  }, [token, toast]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ title: editing ? "Cập nhật giáo viên thành công" : "Thêm giáo viên thành công" });
      setModalOpen(false);
      fetchTeachers();
    } catch (err: unknown) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast({ title: "Xóa giáo viên thành công" });
      fetchTeachers();
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "Đã xảy ra lỗi";
      toast({ title: message, variant: "destructive" });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Danh sách giáo viên trong hệ thống</p>
        </div>
      ...
      <Modal
        ...
        title={editing ? "Chỉnh sửa giáo viên" : "Thêm giáo viên mới"}
        description={editing ? "Cập nhật thông tin giáo viên" : "Nhập thông tin giáo viên mới"}
      >
